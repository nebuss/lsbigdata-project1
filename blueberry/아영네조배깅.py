...
# 각 모델 학습 및 예측
models = {
    "Ridge": Ridge(alpha=0.01),  # 알파 값을 미세 조정
    "Lasso": Lasso(alpha=0.01),  # 알파 값을 미세 조정
    "KNN": KNeighborsRegressor(n_neighbors=7)  # n_neighbors 값을 미세 조정
}
...
weights = {
    "Ridge": 0.5,  # Ridge의 가중치를 조금 더 높게 설정
    "Lasso": 0.5,
    "KNN": 0  # KNN의 가중치를 낮게 설정
}
...
sub_df.to_csv("./submission_berry_bagging_knn0.csv", index=False)
